...
\begin{defin}[Estructuras topológicamente elementales]
Sean $X, Y, Z$ vértices de un grafo. Todo camino o sendero donde $Z$ sea el nodo intermedio entre $X$ e $Y$ se puede clasificar en una de las tres estructuras topológicas elementales:
\begin{enumerate}
    \item Diremos que hay una \textbf{cadena} si el camino tiene estructura $X \rightarrow Z \rightarrow Y$.
    \item Diremos que hay una \textbf{bifurcación (\textit{fork})} si el camino tiene estructura $X \leftarrow Z \rightarrow Y$.
    \item Diremos que hay una \textbf{colisión (\textit{collider})} si el camino tiene estructura $X \rightarrow Z \leftarrow Y$
\end{enumerate}
\textbf{Nota:} no puede ser de otra manera, pues hay dos estados para las flechas y el estado de cadena es simétrico.
\end{defin}
...
Decimos que un grafo dirigido $G = (V, E)$ es una estructura causal en la que cada nodo de $V$ corresponde a una variable aleatoria, y cada arista de $E$ corresponde a una relación funcional entre las variables correspondientes.
\end{defin}

\chapter{Modelos Causales Estructurales}
...
Y diremos que $f_i$ es la ecuación que determina el valor de $X_i$ dados $\text{PA}_i$ y $\text{U}_i$. De manera coloquial, diremos que las variables de $\text{PA}_i$ causan el valor de $X_i$ con ruido $U_i$
\end{defin}

Aquí utilizamos dos conjuntos de variables distintos. No es coincidencia haber escogido $\text{PA}$ para denotar las variables que `causan' el valor de $X_i$, pues se alinean con la Definición~\ref{def:parentesco}. Estas son las variables que tenemos en cuenta en nuestros modelos causales. Por supuesto, no siempre podemos tener en cuenta todas las variables, pues no resulta práctico, así que añadimos y toleramos una influencia del `ruido'. Utilizamos el conjunto de variables $U$ para denotar a este ruido. Es entonces natural definir ahora el `puente' a los \textbf{modelos causales estructurales}. Diremos que existe una \textbf{relación funcional} entre un nodo y sus padres. Notemos que utilizamos $:=$ puesto que $X_i$ es una consecuencia de los valores de los que depende, pero los valores de los que depende no cambian si cambia $X_i$.

Más coloquialmente, supongamos que interpretamos la variable $X$ como `el suelo está mojado'. Si $Y$ es `ha llovido' y $U$ es el ruido. Intuitivamente podríamos decir

\begin{equation*}
X := Y \text{si $U = 0$, $X:= U$ de otra manera}
\end{equation*}

De esta manera, si ha llovido, el suelo estará mojado a no ser que exista algún valor que no consideremos.

\begin{defin}[Estructura causal]
Diremos que una estructura causal de un conjunto de variables $V$ es un DAG en el que cada nodo está asociado a una variable de $V$, y cada arista repreesenta una relación funcional entre un nodo y sus padres.
\end{defin}
