...
Como ya veníamos anticipando, podemos transformar el modelo estructural causal en su grafo correspondiente. Sea $M = \langle U, V, F\rangle$ un modelo estructural causal. Definimos su estructura causal asociada (o DAG asociado) como $G = \langle V \cup U,\bigcup_{i\in \{1, \ldots,n \}} E_i \rangle$, donde $E_i = \{(C, V_i) \mid C \in \text{PA}_i \cup U_i\}$. Es decir, el grafo donde cada enlace representa una relación funcional estructural entre variables.
...
\label{markov-causal}
Sea $M = \langle U, V, F\rangle$ un modelo estructural causal con DAG asociado $G$. 
...
Uno además podría pensar en la cadena en términos de \textbf{cómo de específica} es una información. Supongamos la cadena del modelo estructural causal descrito en la Figura~\ref{fig:grafo-interruptor}, con las interpretaciones del interruptor, el circuito y la bombilla.
...
\subsection{Colisión}
Álvaro y Luis hacen una apuesta. Se tirarán dos monedas. En caso de que las monedas caigan en la misma cara, Álvaro se lleva las dos monedas. En caso contrario, irán a parar al bolsillo de Luis. Digamos $X$ e $Y$ representan las dos monedas, $Z$ representa el ganador de la apuesta. El modelo estructural causal tiene como modelo gráfico el de la Figura~\ref{fig:grafo-interruptor-colision}
