...
\usepackage{tikz}
\usetikzlibrary{decorations.markings, positioning, arrows.meta, bending}
...
Tenemos una población en la que se ha identificado una nueva epidemia. Rápidamente, los doctores responsables del bienestar de dicha población se ponen manos a la obra para intentar descubrir un remedio. Un estudio preliminar observacional sobre una medicación $M$ arroja los datos que podemos ver en la Tabla~\ref{tab:drug-no-gender}.
...
A lo largo del trabajo utilizaremos los conceptos del Apéndice~\ref{bases-conceptuales}

\chapter{Conceptos de teoría de grafos}
\begin{defin}[Orden topológico]
Sea $G$ un grafo con nodos en $V$. Un orden topológico de los nodos de $V$ es un orden $V = \{X_1, \ldots, X_n\}$ tal que para todo par de nodos $X_i, X_j \in V$ tenemos $X_i \rightarrow X_j$ implica $i < j$.
\end{defin}

\begin{teor}
Todo grafo acíclico dirigido tiene un orden topológico.
\end{teor}
Para demostrar este teorema necesitamos el siguiente lema
\begin{lema}
Todo grafo acíclico dirigido debe tener al menos un nodo sin aristas entrantes.
\end{lema}
\begin{proof}
Supongamos por reducción al absurdo un grafo acíclico dirigido con $n$ nodos en el que todo nodo tiene por lo menos una flecha entrante. Escogemos un nodo cualquiera $X_0 \in V$. Como tiene una arista entrante, 
existe $X_1$ tal que $X_1 \to X_0$. Como $X_1$ también tiene una arista entrante, existe $X_2$ tal que $X_2 \to X_1$. Continuando este proceso, construimos una sucesión $X_0, X_1, X_2, \ldots$ Como $V$ es finito, por el principio del palomar existe $i < j$ tal que $X_i = X_j$. Entonces la secuencia $X_j \to X_{j-1} \to \cdots \to X_i = X_j$ es un ciclo dirigido en $G$, contradiciendo que $G$ es un DAG.
\end{proof}

Ahora demostramos el original
\begin{proof}
Vamos por inducción sobre el número de nodos en el grafo $n$. Si $n = 1$ el conjunto de un solo elemento es el orden topológico.

Supongamos que todos los grafos con menos de $n$ nodos tienen un orden topológico. Sea $G = (V, E)$ un grafo acíclico dirigido con $n$ nodos. Consideramos uno de sus nodos sin aristas entrantes, llamémoslo $X_1$. Consideramos el grafo $G' = (V - \{X_1\}, E')$ donde $E'$ es el conjunto de aristas sin las entrantes o salientes de $X_1$. El grafo $G$ es acíclico y dirigido pues quitar nodos no crea ciclos, y tiene $n-1$ nodos.

Consideramos un orden topológico de $G'$, $X_2, X_3, \ldots, X_n$. Entonces, $X_1, X_2, \ldots, X_n$ es un orden topológico de $G$. En efecto, para todo $i \neq 1$ $X_i \to X_j$ implica $i < j$ por la hipótesis inductiva y porque $j \neq 1$ dado que entonces habría una arista entrante en $X_1$. Si $i = 1$ tendríamos $X_1 \rightarrow X_j$ pero como $j \geq 2$ tenemos $i = 1 < 2 \leq j$. 
\end{proof}


\begin{defin}[Padres Markovianos]
\label{def:padres-markovianos}
Sea $V = \{X_1, \ldots, X_n\}$ un conjunto de variables ordenadads y sea $P(v)$ su distribución de probabilidad conjunta. Un conjunto $\text{PA}_j \subseteq V$ se dice que son los padres Markovianos de $X_j$ si $\text{PA}_j$ es el mínimo conjunto de variables predecesoras (es decir $X_i \in \text{PA}_j \rightarrow i < j$) tal que condicionar por los padres Markovianos de $X_j$ es lo mismo que condicionar por todos sus predecesores , es decir:

\begin{equation*}
P(X_j\mid \text{PA}_j) = P(X_j \mid X_1, \ldots X_{j-1})
\end{equation*}
\end{defin}
...
\subsection{Definición de modelo estructural causal}
...
Diremos que una estructura causal de un conjunto de variables $V$ es un grafo acíclico dirigido en el que cada nodo está asociado a una variable de $V$, y cada arista representa una relación funcional entre un nodo y sus padres.
\end{defin}

\begin{defin}[Modelo causal o modelo estructural causal]
...
\end{tikzpicture}
\caption{Modelo estructural descrito. Las variables exógenas junto con las endógenas antecesoras de cada variable determinan el estado del sistema.}
\label{fig:grafo-interruptor}
\end{figure}

Supongamos, por ejemplo, que interpretamos $X$ como `número de minutos tarde que transcurren al despertarse', $Y$ como `número de minutos tarde que llegas a la oficina' y $Z$ como `duración del enfado de un jefe poco permisivo'. Las expresiones concretas que describan las relaciones causales entre estas tres variables no alteran el grafo. Por tanto, la estructura causal de este modelo coincide con la representada en la Figura~\ref{fig:grafo-interruptor}. En consecuencia, cualquier resultado que se derive del modelo gráfico será válido para todo modelo estructural causal cuyo grafo sea ese.

\section{Distribución de probabilidad compatible con un grafo}
Para poder establecer el puente de paso final entre un grafo y sus distribuciones de probabilidad compatibles nos apoyaremos en la condición causal de Markov, así como en el concepto de padres Markovianos (\ref{def:padres-markovianos}).

\begin{defin}[Condición de Markov Causal]
\label{markov-causal}
Sea $M = \langle U, V, F\rangle$ un modelo estructural causal con grafo acíclico dirigido asociado $G$. 
Fijemos un orden topológico de $V$, necesario para la 
definición de padres Markovianos (Definición~\ref{def:padres-markovianos}). 
Diremos que $M$ satisface la \textbf{condición de Markov Causal} 
si para todo $V_j \in V$, sus padres Markovianos coinciden 
con sus padres en $G$.
\end{defin}

\begin{teor}
\label{distro-compat-teorema}
Sea $M$ un modelo estructural causal que satisface la condición de Markov. La distribución conjunta de las variables endógenas puede factorizarse como
\begin{equation*}
P(v_1, \ldots, v_n) = \prod_{i=1}^n P(v_i \mid \text{pa}_i)
\end{equation*}
\end{teor}
\begin{proof}
Sea $V$ el conjunto de variables endógenas de $M$. Podemos escribir un orden topológico de la forma $V = \{V_1, V_2, \ldots, V_n\}$. Por la regla del producto (ver~\ref{regla-del-producto}), la distribución conjunta $P(v_1, v_2, \ldots, v_n)$ factoriza como $\prod_{j=1}^n P(v_j \mid v_1, \ldots, v_{j-1})$.

Al estar en orden topológico, el conjunto $\{V_1, \ldots, V_{j-1}\}$ contiene a los padres de $V_j$ (porque si $V_i \to V_j$ entonces $i < j$). Como el modelo $M$ cumple la condición de Markov causal, los padres de $V_j$ coinciden con sus padres Markovianos. Por definición de padres Markovianos, tenemos
\begin{equation*}
P(v_j \mid v_1, \ldots, v_{j-1}) = P(v_j\mid pa_j)
\end{equation*}

Sustituyendo en el producto $\prod_{j=1}^n P(v_j \mid v_1, \ldots, v_{j-1})$ queda la expresión deseada.
\end{proof}

Finalmente, podemos definir el puente final entre un grafo y una distribución de probabilidad.

\begin{defin}[Distribución compatible]
Diremos que una distribución es compatible con un grafo $G$ si factoriza de acuerdo al Teorema~\ref{distro-compat-teorema}.
\end{defin}

\section{Estructuras topológicas elementales: cadena, bifurcación y colisión: d-separación}
Supongamos que tenemos un grafo dirigido de tres nodos $(X, Y, Z)$ tal que $X$ está conectado con $Z$, cualquiera que sea la dirección de dicha conexión. De forma similar, $Z$ está conectado con $Y$. Entonces, existen cuatro estados posibles.
\begin{enumerate}
...
\subsection{Cadena}
Podríamos aventurarnos a formular afirmaciones sobre la dependencia de las variables. En el caso de la cadena, la intuición nos dice que $Z$ e $Y$ son dependientes, es decir $P(Z = z \mid Y = y) \neq P(Z)$ para algún par $z, y$. Podríamos también aventurarnos a afirmar que $Z$ y $X$ son dependientes. Podemos construir ejemplos `artificiales' donde estas afirmaciones sean falsas (ver Apéndice~\ref{app:independent}).

Uno además podría pensar en la cadena en términos de \textbf{cómo de específica} es una información. Consideremos la cadena del modelo estructural causal descrito en la Figura~\ref{fig:grafo-interruptor}, con las interpretaciones del interruptor, el circuito y la bombilla.

Armados con nuestra intuición, pensamos en la probabilidad de que la bombilla esté encendida dado que el interruptor está en estado `ENCENDIDO'. Esta es, de forma intuitiva, la probabilidad de que no haya anomalías en el circuito.

Supongamos que conocemos el estado del circuito, si está abierto o cerrado. En ese caso, \textbf{nos es indiferente} el estado del interruptor, pues tiene prioridad el efecto del estado del circuito sobre el efecto del estado del interruptor. Esta prioridad viene simbolizada por la longitud del camino de $X$ a $Z$, que es mayor que de $Y$ a $Z$. Si sabemos que el circuito está cerrado, sabremos que la bombilla está encendida independientemente del estado del interruptor.

Más adelante demostraremos que esta intuición es en general cierta: en la cadena $X \rightarrow Z \rightarrow Y$, $Y$ y $X$ son independientes dado $Z$.

\subsection{Bifurcación}
Supongamos la situación $Z$ como `un interruptor ha sido presionado'. La variable $X$ es `el interruptor ha hecho un ruido' y la variable $Y$ es `la bombilla asociada al interruptor está encendida'. De inmediato vemos la estructura causal de la Figura~\ref{fig:grafo-interruptor-fork}.
...
\end{tikzpicture}
\caption{Modelo estructural descrito. El que alguien le dé al interruptor causa que se encienda la bombilla y que suene el interruptor.}
\label{fig:grafo-interruptor-fork}
\end{figure}

Si un observador que no puede oír si el interruptor ha sido accionado observa que la bombilla se enciende, asumirá que el interruptor ha sonado (a menos que exista otro medio por el que la bombilla pueda encenderse, en este caso una variable exógena $U_Y$).

De igual manera, si un observador que no puede ver la bombilla oye que el interruptor emite un sonido, asumirá que la bombilla se ha encendido (a menos que exista otra manera de provocar el sonido del interruptor sin presionarlo).

Supongamos ahora que en cualquiera de las dos situaciones se le dice al observador `el interruptor ha sido presionado'. En la primera, tanto si la bombilla está encendida como apagada, el observador sabrá que el interruptor ha sonado, pues alguien lo ha accionado. Algo similar ocurre en la otra situación, donde independientemente de si se ha oído sonar el interruptor o no, sabremos que la bombilla se ha encendido.

Esto nos da otra propiedad que también demostraremos más adelante: en una bifurcación como la de la Figura~\ref{fig:grafo-interruptor-fork}, $X$ e $Y$ son condicionalmente independientes dado $Z$.

\subsection{Colisión}
Álvaro y Luis hacen una apuesta. Se tirarán dos monedas. En caso de que ambas caigan del mismo lado, Álvaro se lleva las dos monedas. En caso contrario, irán a parar al bolsillo de Luis. Digamos que $X$ e $Y$ representan las dos monedas, $Z$ representa el ganador de la apuesta. El modelo estructural causal tiene como modelo gráfico el de la Figura~\ref{fig:grafo-interruptor-colision}
...
% Nodos
\node (Z) {$Z$};
\node[above left=of Z] (X) {$X$};
\node[above right=of Z] (Y) {$Y$};

% Aristas
\draw[thick, postaction={decorate}] (X) -- (Z);
\draw[thick, postaction={decorate}] (Y) -- (Z);

\end{tikzpicture}
\caption{Modelo estructural descrito. El resultado de las dos monedas decide el ganador de la apuesta.}
\label{fig:grafo-interruptor-colision}
\end{figure}

El resultado de las dos monedas por separado es independiente. Es decir, $X$ e $Y$ son independientes. Si un observador solo conoce el resultado de la moneda $X$, no podrá conocer el resultado de $Y$. Supongamos que le decimos al observador `Álvaro ganó la apuesta'. En ese caso, le damos a conocer la variable $Z$.

Una vez sabe esto, el observador sabrá que el resultado de la moneda $Y$ es el mismo que el de la moneda $X$. El argumento es simétrico para $Y$ y $X$.

Por ello, $X$ e $Y$ son \textbf{independientes} si no se conoce el valor de $Z$, pero se convierten en \textbf{dependientes} cuando conocemos dicho valor. Esto también es cierto para los descendientes de $Z$. Si no conocemos quién ha ganado pero un observador nos dice quién ganó con un 0.75 de fiabilidad, vuelve a romperse la independencia.

\subsection{La d-separación}
Con la intuición clara acerca de las tres topologías elementales, podemos pasar a definir cuándo un camino está bloqueado entre variables. Utilizamos la definición de camino~\ref{def:camino}.

\begin{defin}[d-separación o bloqueo]
Diremos que un camino $p$ entre dos nodos está d-separado o bloqueado por un conjunto de nodos $Z$ si
\begin{enumerate}
    \item $p$ contiene una cadena $x \rightarrow z \rightarrow y$ o una bifurcación $x \leftarrow z \rightarrow y$ tal que el nodo $z$ está en $Z$.
    \item $p$ contiene una colisión $x \rightarrow z \leftarrow y$ tal que el nodo $z$ o cualquier descendiente de $z$ \textbf{no} está en $Z$.
\end{enumerate}
\end{defin}

Si un camino no es d-separado, diremos que es d-conexo. Intuitivamente, un camino d-separado no permite el paso de dependencia condicional entre los extremos del camino. En el ejemplo de la Figura~\ref{fig:d-separation-illustration}, el camino entre $X$ e $Y$ es d-separado si por lo menos una de las siguientes se cumple:
\begin{enumerate}
\item $Z_1$ y $Z_3$ no están ambos en $Z$
\item $Z_2$ está en $Z$
\end{enumerate}
...
% Nodes
\node (X) [label=left:$X$] {};
\node (Z1) [right of=X, label=below:$Z_1$] {};
\node (Z2) [right of=Z1, label=below:$Z_2$] {};
\node (Z3) [right of=Z2, label=below:$Z_3$] {};
\node (Y)  [right of=Z3, label=right:$Y$] {};
...
\end{tikzpicture}
    \caption{En este ejemplo, $X$ e $Y$ estarán d-separados si $Z_1 \notin Z$ o si $Z_3 \notin Z$, o si $Z_2 \in Z$}
    \label{fig:d-separation-illustration}
\end{figure}

Diremos que dos nodos son d-separados por $Z$ si todo camino entre ellos está d-separado por $Z$. Diremos que dos conjuntos $X, Y$ son d-separados si todo todo par de nodos en $X \times Y$ es d-separado. Por fin podemos recoger todo lo sembrado a lo largo de este capítulo en el siguiente teorema.

\begin{teor}
Los conjuntos $X$ e $Y$ son d-separados por un conjunto $Z$ en un grafo acíclico dirigido $G$ si y solo si las variables en $X$ son independientes de las variables en $Y$ condicionando por $Z$ en toda distribución compatible con $G$.
\end{teor}

\begin{proof}
Primero veamos los casos locales, es decir, veamos que la d-separación bloquea la dependencia condicional en las tres topologías elementales del grafo.
\begin{enumerate}
    \item Para la cadena $x \to z \to y$ tal que $z \in Z$. Como estamos suponiendo que satisfacemos la condición de Markov causal, tenemos que $(y \perp ND_y) \mid PA_y$, es decir $(y \perp x )\mid z$.
    \item Para la bifurcación $x \leftarrow z \to y$ tal que $z \in Z$, tenemos que por la misma razón, $x \perp ND_x \mid PA_x$, por lo que $x \perp y \mid z$.
    \item Para la colisión tenemos que $x \to z \leftarrow y$ donde $z \notin Z$. En efecto, como $PA_x = \emptyset$, se cumple $x \perp y\mid \emptyset$ o bien $x \perp y$.
\end{enumerate}

A continuación, debemos demostrar una afirmac
\end{proof}
...
Supongamos tres variables aleatorias $X, Y, Z$ conectadas por un modelo estructural causal en cadena tal que $X \rightarrow Y \rightarrow Z$. Sabemos entonces que existen $f_Y, f_Z$ tales que $f_Y$ determina el valor de $Y$ dependiente del valor de $X$ y análogamente con $f_Z$, $Z$ e $Y$.
