...
Sí podemos afirmar cosas sobre todas las variables. Cambiamos, por claridad la topología de la cadena a $X \rightarrow Y \rightarrow Z$, pero es equivalente.

Para lo que sigue, usaremos el siguiente lema:
\begin{lema}
Supongamos un SCM con DAG asociado $X \rightarrow Y \leftarrow U_Y$, usando notación $f_Y: D_X \times D_{U_Y} \rightarrow D_Y$ entonces
\begin{equation*}
    P(Y = y \mid X = x) = P(f_Y^{-1}(y) \cap \{(x, u_y) \mid u_y \in D_{U_Y}\})
\end{equation*}
\end{lema}
\begin{proof}
...
\begin{lema}
Supongamos que un SCM tiene DAG asociado $X \rightarrow Y \rightarrow Z$, entonces, $X$ y $Z$ son condicionalmente independientes dado $Y$.
\end{lema}
\begin{proof}
Buscamos demostrar que 
\end{proof}
