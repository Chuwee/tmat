...
Sea $M = \langle U, V, F\rangle$ un SCM con DAG asociado $G$. En el contexto de la Definición~\ref{def:padres-markovianos}, decimos que las variables predecesoras de $V_j$ son aquellas variables del grafo que forman parte de los \textbf{no descendientes} de $V_j$ ($\text{ND}_j$). Diremos que el modelo $M$ satisface la condición de Markov causal si para todo $V_j \in V$ sus padres Markovianos coinciden con sus padres del grafo $G$.
\end{defin}
...
Uno además podría pensar en la cadena en términos de \textbf{cómo de específica} es una información. Supongamos la cadena del SCM descrito en la Figura~\ref{fig:grafo-interruptor}, con las interpretaciones del interruptor, el circuito y la bombilla.
...
\end{tikzpicture}
\caption{Modelo estructural del SCM descrito. El que alguien le dé al interruptor causa que se encienda la bombilla y que suene el interruptor.}
...
\subsection{Colisión}
Álvaro y Luis hacen una apuesta. Se tirarán dos monedas. En caso de que las monedas caigan en la misma cara, Álvaro se lleva las dos monedas. En caso contrario, irán a parar al bolsillo de Luis. Digamos $X$ e $Y$ representan las dos monedas, $Z$ representa el ganador de la apuesta. El SCM tiene como modelo gráfico el de la Figura~\ref{fig:grafo-interruptor-colision}
...
\end{tikzpicture}
\caption{Modelo estructural del SCM descrito. El resultado de las dos monedas decide el ganador de la apuesta.}
