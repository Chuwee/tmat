...
\begin{lema}
Todo grafo acíclico dirigido debe tener al menos un nodo sin flechas entrantes.
\end{lema}
\begin{proof}
Supongamos un grafo acíclico dirigido con $n$ nodos en el que todo nodo tiene por lo menos una flecha entrante. Escogemos un nodo cualquiera de dicho grafo. Al tener por lo menos una flecha entrante, podemos escoger uno de los nodos que apunta a él. Continuamos sucesivamente. Si después de $k < n$ iteraciones escogemos un nodo que ya hemos recorrido, entonces hemos terminado.

Si no, entonces después de $n$ iteraciones llegaremos a una lista de nodos recorridos que como máximo puede tener longitud $n$. Repitiendo el proceso una vez más (podemos, pues todo nodo tiene una flecha entrante), habremos recorrido $n + 1$ nodos de una lista de $n$ nodos y, por el principio del palomar, habremos recorrido algún nodo dos veces.

En cualquiera de los dos casos, si seguimos el camino en orden inverso, tendremos un ciclo.
\end{proof}

Ahora demostramos el original
\begin{proof}
Escogemos como primeros nodos en cualquier orden aquellos que no tienen flechas entrantes. Posteriormente, escogemos los hijos del primer nodo, en cualquier orden que se quiera, seguidos de los del segundo nodo... (búsqueda en anchura). Así construimos un orden topológico.
\end{proof}
